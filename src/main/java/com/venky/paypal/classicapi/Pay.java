...
public class Pay {
	...
	public PayResponse chainPay() {
		...
		// Amount to be credited to the receiver's account
		Receiver receiver1 = new Receiver(Double.parseDouble("5.50"));

		// A receiver's email address
		receiver1.setEmail("tivamo.owner@gmail.com");
		...
		// Amount to be credited to the receiver's account
		Receiver receiver2 = new Receiver(Double.parseDouble("1.00"));

		// A receiver's email address
		receiver2.setEmail("tivamo.seller@gmail.com");
		...
		// Amount to be credited to the receiver's account
		Receiver receiver3 = new Receiver(Double.parseDouble("1.00"));

		// A receiver's email address
		receiver3.setEmail("tivamo.cause@gmail.com");

		// Set to true to indicate a chained payment; only one receiver can be a
		// primary receiver. Omit this field, or set it to false for simple and
		// parallel payments.
		receiver3.setPrimary(Boolean.FALSE);
		receiverLst.add(receiver3);
		...
		payRequest = new PayRequest(requestEnvelope, "PAY_PRIMARY",
				"http://localhost/cancel", "USD", receiverList,
				"http://localhost/return");
		
		payRequest.setReceiverList(receiverList);
		payRequest.setFeesPayer("EACHRECEIVER");
		payRequest.setMemo("for purchased id :xxx");
	...
	private PayResponse makeAPICall(PayRequest payRequest) {
		...
		try {
			service = new AdaptivePaymentsService(Pay.class.getResourceAsStream("sdk_config.properties"));
		} catch (IOException e) {
		...
		if (payResponse.getResponseEnvelope().getAck().getValue()
			...
			System.out.println("https://www.sandbox.paypal.com/cgi-bin/webscr?cmd=_ap-payment&paykey=" + payResponse.getPayKey());
		}
		// ### Error Values
		// Access error values from error list using getter methods
		else {
			for(ErrorData error : payResponse.getError()) {
				logger.severe("API Error Message : " + error.getMessage());
			}
		}
		return payResponse;
